...
#define SPIFFS_ADDRESS 0x290000

extern const uint8_t ESP32_bootloader_bin[];
extern const uint32_t ESP32_bootloader_bin_size;
extern const uint8_t ESP32_firmware_bin[];
extern const uint32_t ESP32_firmware_bin_size;
extern const uint8_t ESP32_partition_table_bin[];
extern const uint32_t ESP32_partition_table_bin_size;
extern const uint8_t ESP32_spiffs_bin[];
extern const uint32_t ESP32_spiffs_bin_size;
...
void get_example_binaries(target_chip_t target, example_binaries_t *bins)
{
    if (target == ESP32_CHIP)
    {
        ...
        bins->app.data = ESP32_firmware_bin;
        bins->app.size = ESP32_firmware_bin_size;
        bins->app.addr = APPLICATION_ADDRESS;
        bins->spiffs.data = ESP32_spiffs_bin;
        bins->spiffs.size = ESP32_spiffs_bin_size;
        bins->spiffs.addr = SPIFFS_ADDRESS;
    }
    else
    {
        abort();
